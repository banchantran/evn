<?php
	...
	$url = getParam(1, 'str');
	$query = 'SELECT * FROM `news_detail` WHERE lang="'.$current_lang.'" AND title_url="'.$title_url.'" AND url="'.$url.'" AND sub_url="'.$sub_url.'"';
	...
	//$query = 'SELECT * FROM `news_detail` WHERE lang="'.$current_lang.'" and id<>"'.$row['id'].'" AND url="'.$url.'" AND sub_url="'.$sub_url.'" ORDER BY region, id DESC limit 10';
	//hien thi tu do 
	$query = 'SELECT * FROM `news_detail` WHERE lang="'.$current_lang.'" and id<>"'.$row['id'].'" AND url="'.$url.'" AND sub_url="'.$sub_url.'" ORDER BY RAND(), id DESC limit 10';
	$database->setQuery($query);
