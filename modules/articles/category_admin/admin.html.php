<?php
class HTML_articles_category
{
	function listArticles_cat($rows, $pagging, $start_count)
	{
	?>
		<script >
			...
			{	
				...
				if (res) 
				{
					document.location.href= "<?php echo generate_url('articles_category_admin', array('url'=>$_SESSION['cat'], 'task'=>'delete')); ?>&value="+value;
				}
			}
			...
			{	
				...
				document.location.href= "<?php echo generate_url('articles_category_admin', array('url'=>$_SESSION['cat'], 'task'=>'edit')); ?>&id="+value;
			}
			function countChecked() 
			{
				var nCount = 0;
				dml=document.FormListArticles_cat;
			...
			{
				document.FormListArticles_cat.action.value=pressbutton;
				try
				{
					document.FormListArticles_cat.onsubmit();
				}
				catch(e)
				{}
				document.FormListArticles_cat.submit();
			}
		...
		<form name="FormListArticles_cat" method="post">
		<table class="nostyle" border="0" cellpadding="0" cellspacing="0" width="100%">
			<tbody>
				<tr>
					<td align="left"><h2><?php echo _CATEGORY_ADMIN;?></h2></td>
					<td align="right">
						<a class="toolbar" href="<?php echo generate_url('articles_category_admin', array('url'=>$_SESSION['cat'], 'task'=>'add', 'curPg')); ?>">
						...
						<a class="toolbar" href="#" onclick="return editArticles_cat()">
						...
						<a class="toolbar" href="#" onclick="return deleteArticles_cat()">
				...
				if($rows)
				{
					...
					foreach($rows as $row)
					{
						...
						<tr class="<?php echo $class_row; ?>" valign="top">
							...
							<td><?php echo $i; ?></td>
							<td>
								<?php echo $tab;?><a href="<?php echo generate_url('articles_category_admin', array('url'=>$_SESSION['cat'], 'task'=>'edit', 'id'=>$row['id'])); ?>"><?php echo $row['title']; ?></a>
							</td>
                            <td>
                                <a href="<?php if($row['publish']==2) $status='unpublish'; else $status='publish'; echo generate_url('articles_category_admin', array('url'=>$_SESSION['cat'], 'status'=>$status, 'id'=>$row['id'])) ?>"><?php  $status_all = array('1'=>_UN_PUBLISH, '2'=>_PUBLISH); echo $status_all[$row['publish']];?></a>
							</td>							
							<td>
							...
							<td>
                             ...
                             <td>								
                            <?php echo $tab;?><?php											
								$other_link = generate_url('articles_admin', array('url'=>$row['url'], 'sub_url'=>$row['title_url']));							
	...
	function updateArticles_cat($row='', $all_arr, $error_array)
	{
	?>
		<form name="FormAddArticles_cat" method="post" enctype="multipart/form-data">
		<script>
			function submitbutton(pressbutton)
			{
				document.FormAddArticles_cat.action.value=pressbutton;
				try
				{
					document.FormAddArticles_cat.onsubmit();
				}
				catch(e)
				{}
				document.FormAddArticles_cat.submit();
			}
		</script>
		<table class="nostyle" border="0" cellpadding="0" cellspacing="0" width="100%">
			<tbody>
