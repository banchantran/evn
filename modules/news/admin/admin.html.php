<?php
class HTML_news_detail
{
	function listNews($rows, $pagging, $start_count)
	{
	?>
		<script >
			function deleteNews()
			{	
				...
				if (res) 
				{
					document.location.href= "<?php echo generate_url('news_admin', array('url'=>$_SESSION['cat'], 'sub_url'=>$_SESSION['sub_cat'], 'task'=>'delete', 'curPg')); ?>&value="+value;
				}
			}
			...
			{	
				...
				if (res) 
				{
					document.location.href= "<?php echo generate_url('news_admin', array('url'=>$_SESSION['cat'], 'sub_url'=>$_SESSION['sub_cat'], 'task'=>'delete_image', 'curPg')); ?>&value="+value;
				}
			}
			function editNews()
			{	
				...
				document.location.href= "<?php echo generate_url('news_admin', array('url'=>$_SESSION['cat'], 'sub_url'=>$_SESSION['sub_cat'], 'task'=>'edit', 'curPg')); ?>&id="+value;
			}
			...
			{
				document.FormListNews.action.value=pressbutton;
				try
				{
					document.FormListNews.onsubmit();
				}
				catch(e)
				{}
				document.FormListNews.submit();
			}
		...
		<table class="nostyle"  border="0" cellpadding="0" cellspacing="0" width="100%">
			<tbody>
				<tr>
					<td align="left"><h2><?php echo _NEWS_ADMIN;?></h2></td>
					<td align="right">
						<a class="toolbar" href="<?php echo generate_url('news_admin', array('url'=>$_SESSION['cat'], 'sub_url'=>$_SESSION['sub_cat'], 'task'=>'add', 'curPg')); ?>">
						...
						<a class="toolbar" href="<?php echo generate_url('news_category_admin', array('url'=>$_SESSION['cat'], 'sub_url'=>$_SESSION['sub_cat']));?>">
				...
				<table class="adminlist" width="100%">
					<tbody>
						<tr>
                            ...
                            <th class="title" width="11%">Tin nóng
                              <select name="host_news" onchange="this.form.submit()">
                                <?php echo get_option(array('0'=>_ALL, '1'=>_UN_PUBLISH, '2'=>_PUBLISH), getParam('host_news', 'int', 0)); ?>
                              </select> </th>
				...
				if($rows)
				{
					...
					foreach($rows as $row)
					{
						...
						<tr class="<?php echo $class_row; ?>" valign="top">
							...
							<td><?php echo $i; ?></td>
							<td>
								<a href="<?php echo generate_url('news_admin', array('url'=>$row['url'], 'sub_url'=>$row['sub_url'], 'news_url'=>$row['news_url'], 'task'=>'edit', 'id'=>$row['id'])); ?>"><?php echo $row['title']; ?></a>
                            ...
                            <td>
                                <a href="<?php if($row['publish']==2) $status='unpublish'; else $status='publish'; echo generate_url('news_admin', array('url'=>$_SESSION['cat'], 'sub_url'=>$_SESSION['sub_cat'], 'status'=>$status, 'id'=>$row['id'])) ?>"><?php  $status_all = array('1'=>_UN_PUBLISH, '2'=>_PUBLISH, '3'=>_PRIVATE); echo $status_all[$row['publish']];?></a>
							</td>
                            <td>
                                <a href="<?php if($row['host_news']==2) $status='no_host_news'; else $status='host_news'; echo generate_url('news_admin', array('url'=>$_SESSION['cat'], 'sub_url'=>$_SESSION['sub_cat'], 'status'=>$status, 'id'=>$row['id'])) ?>"><?php  $status_all = array('1'=>_UN_PUBLISH, '2'=>_PUBLISH); echo $status_all[$row['host_news']];?></a>
							</td>
							<td>
			...
			<tbody>
				<tr>
					...
					<td align="right">
										...
										<a class="toolbar" href="<?php echo generate_url('news_admin', array('url'=>$_SESSION['cat'], 'sub_url'=>$_SESSION['sub_cat'])); ?>">
											<img src="<?php echo SITE_PATH;?>themes/admin/images/cancel.png" alt="<?php echo _CANCEL;?>" name="<?php echo _CANCEL;?>" title="<?php echo _CANCEL;?>" align="middle" border="0">
										</a>
					</td>
				</tr>
				...
				<table class="adminform" width="100%">
					<tbody>
                        ...
                        <tr>
							...
							<td colspan="2">
								<select name="host_news"><?php $publish_array = array('1'=>_UN_PUBLISH, '2'=>_PUBLISH, '3'=>_PRIVATE); echo get_option($publish_array, getParam('host_news', 'int', $row['host_news'])); ?></select>
							</td>
						</tr>
                        <tr>
							<td width="130px">
							...
							<td>
								<?php
									...
									{
										$icon = get_icon($row, 'small', 'news_detail');
										if($icon)
											$option = 2;
									}
								?> 
